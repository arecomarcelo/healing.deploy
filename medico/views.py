...
def consultas_medico(request):
    ...
    consultas_hoje = consultas_hoje.filter(data_aberta__data__gte=hoje)
    consultas_hoje = consultas_hoje.filter(data_aberta__data__lt=hoje + timedelta(days=1))

    consultas_restantes = Consulta.objects.exclude(id__in=consultas_hoje.values('id'))
    consultas_restantes = consultas_restantes.filter(data_aberta__user=request.user)
...
def consulta_area_medico(request, id_consulta):
    ...
    if request.method == "GET":
        ...
        return render(request, 'consulta_area_medico.html',
                      {'consulta': consulta, 'documentos': documentos, 'is_medico': is_medico(request.user)})
...
def finalizar_consulta(request, id_consulta):
    ...
    if request.user != consulta.data_aberta.user:
        messages.add_message(request, constants.ERROR, 'Esta Consulta não pertence ao Usuário Logado!')
    else:
        consulta.status = "F"
        consulta.save()
